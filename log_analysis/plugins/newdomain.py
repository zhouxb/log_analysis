...
class NewDomainAnalysis(IPlugin):
    OUTPUTPATH = os.path.join(settings.APP_OUTPUT_DIR, "newdomain")
    ...
    def analysis(self, entries):
        ...
        candidate_domains = {entry[dnslog.DOMAIN] : 
                             round_minutes_by_5(entry[dnslog.DATE]).strftime(dnslog.formats[0]) 
                             for entry in entries}
        cPickle.dump(candidate_domains, 
                     open(os.path.join(NewDomainAnalysis.OUTPUTPATH, "%s.pickle" % uuid.uuid4().hex), "w"), 
                     cPickle.HIGHEST_PROTOCOL)

    def collect(self):
        new_domains, candicates = self.do_collect()
        self.save_whole_result(new_domains, candicates)

        logging.info("newdomain analysis finished successfully")

    @util.ensure_directory(OUTPUTPATH)
    def do_collect(self):
        results = map(util.load_and_delete, 
                      filter(lambda f: not f.endswith("domaincache.pickle"), 
                             util.listdir(NewDomainAnalysis.OUTPUTPATH)))
        ...
        return new_domains, candicates
    def save_whole_result(self, new_domains, candicates):
        ...
        if batch_data:
            ...
            cPickle.dump(new_domains | self.domain_cache , 
                         open(os.path.join(NewDomainAnalysis.OUTPUTPATH, "domaincache.pickle"), "w"), 
                         cPickle.HIGHEST_PROTOCOL)
